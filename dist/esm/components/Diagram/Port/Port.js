...
var Port = function Port(props) {
  ...
  onDrag(function (event, info) {
    if (onDragNewSegment) {
      ...
      var from = getRelativePoint(info.start, [canvas.x + panVal.x, canvas.y + panVal.y]);
      var to = getRelativePoint([event.clientX, event.clientY], [canvas.x + panVal.x, canvas.y + panVal.y]);
